...
describe("notificação mínima sem PII", () => {
  it("gera Message-ID estável e link HTTPS do Admin", () => {
    ...
    const input={notificationKey,leadId:7,receivedAt:new Date("2026-08-30T12:00:00Z"),adminBaseUrl:"https://staging.crescimentovertical.com"};
  ...
  it("não transporta campos pessoais, pixel, script ou anexo", () => {
    const result=buildLeadNotification({notificationKey:randomUUID(),leadId:1,receivedAt:new Date("2026-08-30T12:00:00Z"),adminBaseUrl:"https://staging.crescimentovertical.com"});
  ...
  it("confirma aceite SMTP sem registrar envelope ou conteúdo", async () => {
    const notification=buildLeadNotification({notificationKey:randomUUID(),leadId:1,receivedAt:new Date(),adminBaseUrl:"https://staging.crescimentovertical.com"});
